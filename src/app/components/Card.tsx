...
const Card = ({
  ...
  reverse, // Tells us if we want the flex direction to be reversed or not
}: {
  post: Post;
  reverse: boolean;
}) => {
    ...
    <div className="bg-[#FFFAEF] border-2 border-solid border-[#fbe09f] lg:w-full m-2 rounded-lg">
      <Link href={`/${post.slug}`}>
        <div
          ...
          } lg:flex-col justify-between h-full`}
        >
          ...
          <div
            ...
            } font-bold m-6 lg:m-4 lg:text-left flex flex-col justify-between`}
          >
            <div>
              <p className="text-red-800 tracking-tighter font-inconsolata text-sm">
                {moment(post.createdAt).format("MMM DD, YYYY")}
              </p>
              <h3 className="text-[#171520]">{post.title}</h3>
            </div>
            <div className="mt-24">
              <p className="text-stone-500 tracking-tighter font-inconsolata text-sm">
                Read Full Article
              </p>
            </div>
          </div>
