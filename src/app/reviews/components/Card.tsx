...
const Card = ({
  ...
  return (
    <div className="bg-[#750308] my-3 md:m-2 lg:my-4 rounded-xl">
      <Link href={`/${post.slug}`}>
        <div className={`flex justify-start h-full`}>
          ...
          <div className="text-left font-bold m-6 2xl:m-8 flex flex-col justify-between">
            <div>
              <p className="tracking-tighter font-inconsolata text-xs md:text-sm xl:text-md">
                {moment(post.createdAt).format("MMM DD, YYYY")}
              </p>
              <h3 className="text-md md:text-lg xl:text-2xl">{post.title}</h3>
            </div>
            <div className="mt-12">
              <p className="text-red-400 tracking-tighter font-inconsolata text-xs md:text-sm">
                Read Full Article
              </p>
            </div>
          </div>
